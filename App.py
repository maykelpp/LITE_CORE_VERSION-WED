            ...
            statusDiv.innerHTML = `
                <div class="status-content">
                    ...
                    <div class="status-text">
                        <strong>Procesando descarga...</strong>
                        <small>Esto puede tardar unos segundos. No cierres esta ventana.</small>
                    </div>
                </div>
            `;
                ...
                if (data.success) {
                    ...
                    statusDiv.innerHTML = `
                        <div class="status-content">
                            ...
                            <div class="status-text">
                                <strong>¡Descarga completada exitosamente!</strong>
                                <small>Tu archivo está listo para descargar</small>
                                <br>
                                <a href="/file/${encodeURIComponent(data.filename)}" 
...
def sanitize_filename(filename):
    """Limpia y normaliza nombres de archivo"""
    filename = re.sub(r'[<>:"/\\|?*]', '', filename)
    filename = re.sub(r'\s+', ' ', filename).strip()
    return filename[:200]

def find_downloaded_file(title, download_dir):
    """Busca el archivo descargado en el directorio"""
    normalized_title = sanitize_filename(title)
...
def download():
    try:
        ...
        if format_type == 'audio':
            filename_template = f'%(title)s_{timestamp}.%(ext)s'
            ydl_opts = {
                **base_opts,
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            downloaded_file = find_downloaded_file(title, DOWNLOAD_FOLDER)
            
            if not downloaded_file or not os.path.exists(downloaded_file):
                return jsonify({
                    'success': False,
                    'error': 'Archivo descargado no encontrado. Verifica la carpeta downloads/'
                })
...
def serve_file(filename):
    ...
    try:
        ...
        return jsonify({
            'success': False,
            'error': f'Error al enviar archivo: {str(e)}'
        }), 500
